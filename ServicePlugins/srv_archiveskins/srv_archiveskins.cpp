...
{
	NodeDescriptor desc = { L"Skins", L"Archive.org Winamp Skins", NULL, CAP_CUSTOMDIALOG };
	return desc;
}
...
void clearList(HWND hwnd)
{
	...
	previewImageList = ImageList_Create(180, 228, ILC_COLOR24, pageLength, 0);
...
void addItemToList(HWND hwnd, SkinInfo skinInfo)
{
	...
	wsprintf(thumbFile, L"%swmp_archiveorg_cache\\%s.bmp", tempPath, skinInfo.id);
	...
	lvi.pszText = (LPTSTR)(skinInfo.title);
	lvi.cchTextMax = lstrlenW(skinInfo.title);
...
void setToPage(HWND hwnd, int page)
{
	...
	wsprintf(cacheDirName, L"%swmp_archiveorg_cache", tempPath);

	wchar_t helperCmd[1024];
	wsprintf(helperCmd, L"\"%s\\archiveskins_helper.exe\" page %d %d \"%s\"", myDirectory, currentPage, pageLength, cacheDirName);
	...
	while (std::getline(pageStream, line))
	{
		...
		const wchar_t* id = _wcsdup(line.c_str());
		std::getline(pageStream, line);
		const wchar_t* title = _wcsdup(line.c_str());
		std::getline(pageStream, line);
		const wchar_t* filename = _wcsdup(line.c_str());
		SkinInfo si = { id, title, filename };
...
static BOOL view_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case IDC_KEEP:
	{
		if (currentSkin != -1)
		{
			...
			{
				wchar_t cacheFileName[1024];
				wsprintf(cacheFileName, L"%swmp_archiveorg_cache\\%s", tempPath, fileList[currentSkin].filename);
	...
	case IDC_PREVSKIP:
	{
		...
		{
			std::thread bgThread(setToPage, hwnd, currentPage - pageSkip); //setToPage(hwnd, currentPage - pageSkip);
			bgThread.detach();
		}
	}
	break;
...
static BOOL list_OnNotify(HWND hwnd, int wParam, NMHDR* lParam)
{
	if (lParam->code == LVN_ITEMACTIVATE)
	{
		...
		if (!PathFileExists(skinFile))
		{
			// Download to temp
			wchar_t cacheFileName[1024];
			wsprintf(cacheFileName, L"%swmp_archiveorg_cache\\%s", tempPath, fileList[lpnmia->iItem].filename);

			wchar_t helperCmd[1024];
			wsprintf(helperCmd, L"\"%s\\archiveskins_helper.exe\" download \"%s\" \"%s\" \"%s\"", myDirectory, fileList[lpnmia->iItem].id, fileList[lpnmia->iItem].filename, cacheFileName);
			runProcessInBackground(helperCmd);
...
HWND GetCustomDialog(HWND _hwndWinampParent, HWND _hwndLibraryParent, HWND hwndParentControl, wchar_t* skinPath)
{
	...
	GetOption(myServiceID, L"pagelength", L"10", pageLengthOpt, MAX_PATH);
